...
7. **Multiline + auto-dedent**. Lexer regex spans newlines (unterminated → EOF error); parser
   ...
   - **Decided — content checks belong to the parser.** The lexer's contract ends at the token: it
     captures the literal whole, delimiters included, and its tests assert the raw slice plus the
     fact that the surrounding indent/dedent stream is undisturbed. What the content *becomes* —
     delimiter stripping, auto-dedent, segment splitting — is asserted in the parser's tests against
     the AST. Splitting it the other way would have both layers testing the same transformation.
   - **Decided — tabs and CRLF.** A tab in a line's leading whitespace is a lex error ("Tabs are not
     allowed in indentation."); pinp's layout is space-based, and the alternative was a tab silently
     reading as no indent at all. A tab elsewhere on a line — between tokens, or inside a literal —
     stays ordinary. CRLF is **accepted**: `\r\n` terminates a line like `\n`, and the `\r` of a CRLF
     pair is dropped from a multiline literal's content, so a file checked out on Windows compiles
     and yields identical values. A lone `\r` remains an error rather than a guess.
   - **Coverage review done.** `runtime/string.rs` is at **94.9% lines** over the unit tests (from
     ~81% at step 2), and the project totals **95.4% lines / 91.4% regions** across the whole suite.
     Everything reachable is now exercised: the step-2 gaps (`pinp_meminfo`, the concat paths) are
     closed, and two further ones found during this review — `pinp_str_len` reading the *heap*
     length field, and `pinp_meminfo` itself — got targeted unit tests. The only uncovered lines
     left are the two that cannot be reached from a test: the over-long guard (behind the
     `#[ignore]`d `over_long_string_errors`) and the out-of-memory check, which is documented in
     place as deliberately untestable.
     - *Note for future coverage runs:* measure `runtime/string.rs` with `cargo llvm-cov --lib`.
       A whole-suite run reports several of its `#[no_mangle]` functions as uncovered — an
       artifact of merging counters across the integration-test binaries, which link those symbols
       without calling them. The functions are genuinely covered by the unit tests.
