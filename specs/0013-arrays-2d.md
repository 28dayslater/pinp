...
`_` is a valid identifier and may appear in any index-binder position to signal "not needed":

```
for row, _, val in matrix    # row index and value; column index discarded
for _, val in arr            # value only, index discarded
for _, _, val in matrix      # value only; both index binders discarded (silly but valid)
```

The parser treats `_` like any other identifier. Sema must never emit an unused-variable
diagnostic for a binder named `_`; it is conventional don't-care in Python and Rust alike.

**The value binder (always last) must not be `_`** — discarding the value defeats the purpose
of array iteration; a range loop over indices expresses that intent more clearly. Sema rejects:

| Form | Error |
|---|---|
| `for _ in ary1d` | Value binder must not be `_`. |
| `for idx, _ in ary1d` | Value binder must not be `_`. |
| `for _, _ in ary1d` | Value binder must not be `_`. |
| `for row, _, _ in ary2d` | Value binder must not be `_`. |
| `for _, _, _ in ary2d` | Value binder must not be `_`. |
...
///   • an `Int` expression → index that dimension (Bool rejected)
...
- Classify each selector after resolving `FullExtent`:
  - Scalar (`Int`) → **index**: will be runtime-checked. `Bool` is rejected — `mat[true, 0]` is almost certainly a bug.
...
Defining a user function named `identity` is a sema error:
`SemaError::Type("Cannot define a function named `identity`: conflicts with a built-in.")`.
The check is in `analyze_func`, before the body is typed.
...
## Step 13 — Codegen: matrix literal + host readback ([src/codegen/expr.rs](src/codegen/expr.rs), [src/codegen/mod.rs](src/codegen/mod.rs))
...
### `PinpJit::run` — Matrix result

Add the `PinpType::Matrix` arm to the `run()` result match (identical to `Array`, with an extra
dimension):

```rust
PinpType::Matrix(elem_type, rows, cols) => {
    let matrix_ptr = result_buf as *const u8;
    let total = rows * cols;
    let elements = (0..total).map(|i| unsafe {
        match elem_type {
            ArrayElementType::Int   => PinpValue::Int(*matrix_ptr.add(i * 8).cast::<i64>()),
            ArrayElementType::Float => PinpValue::Float(*matrix_ptr.add(i * 8).cast::<f64>()),
            ArrayElementType::Bool  => PinpValue::Bool(*matrix_ptr.add(i) & 1 != 0),
        }
    }).collect();
...
## Step 19 — Negative indexing ([src/sema/](src/sema/), [src/codegen/expr.rs](src/codegen/expr.rs))

Python-style negative indices: `i < 0` addresses element `len + i` from the start, so `-1` is
the last element and `-len` is the first. No parser change is required — a negative index already
parses as a `UnOp::Neg` expression and resolves to `PinpType::Int`.

### Scope

Applies to all **scalar index positions** in read and write:

| Site | Position(s) |
|---|---|
| `arr[i]` read | `i` |
| `arr[i] = v` write | `i` |
| `mat[r, c]` read | `r`, `c` (independently) |
| `mat[r, c] = v` write | `r`, `c` (independently) |

**Slices are not affected.** Literal-bound slice ranges remain non-negative; `arr[-2..-1]` is
out of scope.

### Sema — compile-time check for literal indices

When a scalar index expression is `Node::Int(val)` and the target's length is known at compile
time, sema must validate the effective index:

```
effective = if val < 0 { val + len as i64 } else { val }
```

If `effective < 0 || effective >= len` → `SemaError::Type("Array index out of bounds.")`.

This extends the same compile-time guarantee that already exists for slice bounds to scalar
literal indices. A helper shared by all four sites:
...
Applied in:
- `Node::Index` (read) — after confirming `index_type == Int`
- `Node::Index2D` (read) — for both row and col
- `indexed_assign_check` (write 1D) — after `index_type == Int` guard
- `indexed_assign2d_check` (write 2D) — for both row and col

Non-literal indices (variables, expressions) are not checked at sema time; the runtime
normalization catches out-of-bounds there.

### Codegen — runtime normalization

For all scalar index values (literal or not), emit a normalize step before the existing bounds
check. This is a two-instruction sequence: one `icmp slt` and one `select`:

```
effective = select(i < 0, i + len, i)
```

LLVM constant-folds this away for literal indices (the case already vetted by sema), so there
is no runtime overhead for literal accesses.

A private helper encapsulates the pattern:
...
Applied in two functions (covering all four sites through delegation):
- `bounds_check_and_gep` (1D) — normalize before the `SLT 0` / `SGE n` pair
- `bounds_check_and_gep_2d` (2D) — normalize `row_val` before the row check, `col_val` before the col check

### Tests

#### Sema unit tests (`src/sema/tests.rs`)

- `arr[3]` on a 3-element array → `SemaError::Type` (positive OOB, now caught at compile time)
- `arr[-4]` on a 3-element array → `SemaError::Type` (negative OOB)
- `arr[-3]` on a 3-element array → valid (aliases index 0)
- `mat[2, 0]` on a `2×3` matrix → `SemaError::Type` (row OOB)
- `mat[0, -4]` on a `2×3` matrix → `SemaError::Type` (col OOB)
- `arr[-3] = 99` on a 3-element array → valid
- `arr[-4] = 99` on a 3-element array → `SemaError::Type`

#### End-to-end tests (`tests/matrices.rs`)

**1D reads**
- `arr[-1]` on `[10, 20, 30]` → `30`
- `arr[-3]` on `[10, 20, 30]` → `10`
- Runtime: variable index `i = 0 - 4; arr[i]` on a 3-element array → runtime error

**1D writes**
- `arr[-1] = 99` on `[10, 20, 30]` → last element is 99; others unchanged
- Runtime: `i = 0 - 4; arr[i] = 99` on a 3-element array → runtime error

**2D reads**
- `mat[-1, -1]` on `[1,2; 3,4]` → `4` (bottom-right)
- `mat[0, -1]` on `[1,2,3; 4,5,6]` → `3` (last column of row 0)
- `mat[-1, 0]` on `[1,2; 3,4; 5,6]` → `5` (first column of last row)
- Runtime: `r = 0 - 4; mat[r, 0]` on a 2-row matrix → runtime error

**2D writes**
- `mat[-1, -1] = 99` on `[1,2; 3,4]` → element `[1,1]` is `99`; others unchanged
...
- `arr[3]` on a 3-element array → sema error (positive OOB literal scalar index).
- `arr[-4]` on a 3-element array → sema error (negative OOB literal scalar index).
- `arr[-3]` on a 3-element array → valid.
- `mat[2, 0]` on a `2×3` matrix → sema error (row OOB).
- `mat[0, -4]` on a `2×3` matrix → sema error (col OOB).
...
- `for row, _, val in mat` gives correct row/value pairs; `_` column is silently discarded.
- Loop variable write → sema error (enforced by existing guard).
- `arr[-1]` on a 3-element array → last element.
- `arr[-4]` on a 3-element array → runtime error (negative OOB).
- `arr[-1] = 99` → last element updated, others unchanged.
- `mat[-1, -1]` on a 2×2 matrix → bottom-right element.
- `mat[-1, -1] = 99` → bottom-right element updated.
- Variable negative index OOB → runtime error.

---

## Step 20 — Loose ends

Items discovered during implementation that did not fit earlier steps.

### Don't-care binder `_`

`_` already parses without change (it is a plain identifier). Three additions are needed:

**Parser tests** (`src/parser/tests.rs`) — all must parse successfully:

```rust
fn for_array_underscore_dont_care_binder() {
    // Index binder discarded; value kept.
...
fn for_array_both_index_binders_discarded() {
    // Both index binders discarded — silly but valid; duplicate `_` must not be rejected.
    let ast = parse_ok(indoc! {"
        for _, _, val in matrix
            val
    "});
    assert!(matches!(&ast.top_level[0], TopLevel::Stmt(Stmt::ForArray { .. })));
}
```

**Sema tests** (`src/sema/`) — these must be sema errors (value binder is `_`):

- `for _ in ary1d` → `SemaError::Type("Value binder must not be `_`.")`
- `for idx, _ in ary1d` → same
- `for _, _ in ary1d` → same
- `for row, _, _ in ary2d` → same
- `for _, _, _ in ary2d` → same

**Sema rule** (`src/sema/analyzer.rs`, inside `ForArray` handling): after validating binder
count, check `binders.last() == "_"` and return the error above. When emitting any future
unused-variable diagnostic, skip binders whose name is `"_"`.

### `identity` codegen: replace n² stores with memset + n diagonal stores

The current `gen_identity` emits one store per element — n² stores for an n×n matrix. For
large n this makes the LLVM IR module enormous and JIT compilation proportionally slow.

**Required fix** (`src/codegen/expr.rs`, `gen_identity`):

1. Emit one `memset(ptr, 0, n*n*element_size)` to zero the entire allocation (call the
   libc `memset` or LLVM's intrinsic `llvm.memset`).
2. Emit exactly `n` stores — one per diagonal position `i*n + i` — with the constant `1`
   or `1.0`. Total IR instructions: `n + 2` (alloc + memset + n diagonal stores), regardless
   of matrix size.

This remains branch-free and all diagonal values are still compile-time constants; only the
zero-fill changes from n²−n explicit stores to a single memset.

E2e tests: `identity(3, int)` and `identity(3, float)` already cover correctness; add a smoke
test that `identity(100, float)` compiles and returns a 100×100 matrix with correct diagonal.

### Compound assignment on indexed targets

`ary[idx] += 1` is currently a parse error — the parser only accepts `=` for `IndexedAssign`
and `IndexedAssign2D`; compound operators (`+=`, `-=`, `*=`, `/=`, …) are not wired up.

The parser already handles compound ops for plain names via `Stmt::Assign` (expanding into a
`Bin` read + write). Indexed targets use a different approach: the compound operator is stored
directly in the AST rather than expanding to a read expression, so the index is never
evaluated twice.

- `ary[idx] += rhs` → `IndexedAssign { target, index, value: rhs, compound_op: Some(Add) }`
- `mat[r, c] *= rhs` → `IndexedAssign2D { target, row, col, value: rhs, compound_op: Some(Mul) }`
- Codegen loads the current element through the already-computed `elem_ptr`, applies the op,
  and stores the result. The index expression is evaluated exactly once.

Parser tests must cover: `ary[i] += 1`, `ary[i] -= 1`, `mat[r, c] *= 2`.
Sema tests must cover: type mismatch after compound op (e.g. `ary[i] += 1.5` on Int array),
and division on an Int target (`ary[i] /= 2` is always an error since `/` yields Float).
