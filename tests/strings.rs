...
#[test]
fn a_multiline_literal_round_trips_dedented() {
    // The literal is indented to match the code around it; that indentation is layout, not value.
    assert_eq!(
        eval_str(indoc! {"
            x = 'first
                 second
...
#[test]
fn a_multiline_literal_counts_its_newlines() {
    assert_eq!(
        eval_int(indoc! {"
            x = 'ab
                 cd'
        ...
        "}),
        5,
        "two lines of two bytes plus the newline between them"
    );
}

#[test]
fn indentation_beyond_the_common_prefix_is_content() {
    assert_eq!(
        eval_str(indoc! {"
            x = 'head
                   indented
...
#[test]
fn a_multiline_literal_works_inside_a_function() {
    assert_eq!(
        eval_str(indoc! {"
            banner(): str is
                text = 'one
...
#[test]
fn a_multiline_fstring_interpolates_and_dedents() {
    assert_eq!(
        eval_str(indoc! {"
            name = 'world'
            n = 2
...
#[test]
fn multiline_literals_compare_and_concatenate() {
...
#[test]
fn a_long_multiline_literal_reaches_the_heap() {
    // Past the inline maximum, so the embedded newlines survive a heap round-trip too.
    assert_eq!(
        eval_str(indoc! {"
            x = 'aaaaaaaaaa
                 bbbbbbbbbb'
...
#[test]
fn crlf_source_produces_the_same_value() {
    // A file checked out on Windows compiles, and no `\r` leaks into the value.
    let unix = "x = 'first\n     second'\nx";
    assert_eq!(eval_str(unix), "first\nsecond");
    assert_eq!(eval_str(&unix.replace('\n', "\r\n")), "first\nsecond");
}
